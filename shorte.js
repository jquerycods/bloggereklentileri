    ...
    function urltinyfyprompt_callback(response) {
      ...
      if(response.tinyurl && response.tinyurl!="")
        ...
        alert("Kısa URL alınamadı, daha sonra tekrar deneyin." + ((response.error && response.error != "") ? ("nError: " + response.error) : ""));
    }
    function shortenUrl(elm, long_url, service) {
      waitElm=elm.parentNode.nextSibling;
      if(waitElm) waitElm.style.display="inline";
      loadscript("http://bloggereklentileri.blogspot.com/"+service+"?unify=1&url="+encodeURIComponent(long_url) + "&callback=urltinyfyprompt_callback");
    }